 ...
 *
 * @author Kevin Seim - modified by Choi HyoungKi
 * @since 2.0
 */
public class FieldPadding {
    ...
    public String pad(String text) {
        return pad(text, "utf-8");
    }

    public String pad(String text, String encoding) {
        String res = null;
        try {
            int currentLength;
            if (text == null) {
...
//                currentLength = 0;
            ...
            if (length < 0) {
                ...
                if (currentLength == length) {
                    ...
                    return new String(src, 0, length, encoding);
                }
            }
            ...
            if (justify == FieldPadding.LEFT) {
                ...
                System.arraycopy(pad.getBytes(), 0, tgt, src.length, remaining);
            } else {
                System.arraycopy(pad.getBytes(), 0, tgt, 0, remaining);
                System.arraycopy(src, 0, tgt, remaining, src.length);
            }
            res = new String(tgt, 0, length, encoding);
        }catch(Exception e){
            e.printStackTrace();;
        }
        return res;
